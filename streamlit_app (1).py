...
# --- WIND DATA LOAD (FROM HUGGING FACE) --- #
import polars as pl
import pandas as pd

wind_data = {}

base_wind_path = "https://huggingface.co/datasets/omalljess/reef-metocean-data/resolve/main/wind"

wind_files = {
    "High-Risk-varyingWL-Example_B": f"{base_wind_path}/Non_Prevailing_Wind_Examples.csv",
    "Med-Risk-varyingWL-HR": f"{base_wind_path}/MedRisk_wind.csv",
    "High-Risk-varyingWL-HR": f"{base_wind_path}/HighRisk_wind.csv",
    "High-Risk-varyingWL-NonHR": f"{base_wind_path}/HighRisk_wind.csv",
}

def load_csv_with_retry(url, retries=3, delay=2):
    for i in range(retries):
        try:
            return pl.read_csv(url, low_memory=True)
...
@st.cache_data
def load_wind_data(url):
    return load_csv_with_retry(url)

for key, path in wind_files.items():
    try:
        wind_data[key] = load_wind_data(path)
        print(f"Loaded wind: {key} ({wind_data[key].shape})")

    except Exception as e:
        print(f"Wind load failed for {key}: {e}")

#base_image_path = "https://huggingface.co/datasets/omalljess/reef-metocean-data/resolve/main/images"
#berth_wind_img = f"{base_image_path}/Berth_op_controlloing_wind_speeds.png"
#ship_coords_img = f"{base_image_path}/ship_coordinates.png"
#mooring_layout_img = f"{base_image_path}/mooring_layout.png"

base_image_path = "https://huggingface.co/datasets/omalljess/reef-metocean-data/resolve/main"

berth_wind_img = f"{base_image_path}/Berth_op_controlloing_wind_speeds.png?download=true"
ship_coords_img = f"{base_image_path}/ship_coordinates.png?download=true"
mooring_layout_img = f"{base_image_path}/Mooring_layout.png?download=true"

#base_data_path = "/content/drive/MyDrive/REEF_Metocean_Data_R2/Tabletop_Metocean_cases"
...
def load_data():
    ...
    for folder_path in dataset_folder_names:

        # ---------------------------------------------------------
        # FIXED: combined files now live in base_data_path only
        # ---------------------------------------------------------
        url = f"https://huggingface.co/datasets/omalljess/reef-metocean-data/resolve/main/{folder_path}_combined.csv.gz"

        try:
           df = pl.read_csv(url)
        except Exception as e:
            st.warning(f"Missing or failed to load combined file for {folder_path}: {e}")
            continue
        ...
        try:
            ...
            if not current_selected_cols:
                st.warning(f"No matching columns in {folder_path}")
                continue

            df = df.select(current_selected_cols + ["source_file", "source_folder", "file_date"])

            # ---------------------------------------------------------
            # KEEP YOUR EXACT EXCURSION LOGIC (UNCHANGED)
            # ---------------------------------------------------------
            if X_EXCURSION_COL in df.columns and \
               Y_EXCURSION_COL in df.columns and \
               Z_EXCURSION_COL in df.columns:
                ...
                df = df.with_columns([
                    ...
                    "_temp_original_X_excursion",
                    "_temp_original_Y_excursion",
                    "_temp_original_Z_excursion"
                ])

                if SIM_WATER_SURFACE_Z_COL in df.columns:
                    df = df.with_columns([
                        # MLAB
            ...
            all_dataset_folder_dfs.append((folder_path, df))

        except Exception as e:
            st.error(f"Failed loading {folder_path}: {e}")

    return dataset_folder_names, all_dataset_folder_dfs
...
if selected_folder_name in wind_files:

    current_file_path = wind_files[selected_folder_name]

    current_time_col_excel = "Time [UTC]"
    current_knots_col_excel = "Knots"
    current_udir_col_excel = "Udir [° TN]"
    current_hs_col_excel = "Sea Hs [m]"
    current_tp_col_excel = "Sea Tp [s]"

    current_df_excel = None
    current_min_timestamp_excel = None

    try:
        current_df_excel = load_wind_data(current_file_path)
    except Exception as e:
        st.error(f"Error loading wind data for {selected_folder_name}: {e}")
    ...
    if current_df_excel is not None:
        ...
        try:
            current_df_excel = current_df_excel.with_columns(
                pl.col(current_time_col_excel).str.strptime(pl.Datetime, strict=False)
            )
        except Exception:
            pass

        if current_time_col_excel not in current_df_excel.columns:
            st.error(f"Error: '{current_time_col_excel}' not found in data.")
            current_df_excel = None

        else:

            current_min_timestamp_excel = current_df_excel.select(
                pl.col(current_time_col_excel).min()
            ).item()

            current_df_excel = current_df_excel.with_columns(
                (
                    (pl.col(current_time_col_excel) - pl.lit(current_min_timestamp_excel))
                    .dt.total_seconds()
                    .alias("continuous_time_s_excel")
                )
            )

            current_df_excel = current_df_excel.with_columns(
                (pl.col("continuous_time_s_excel") / (3600 * 24))
                .alias("continuous_time_d_excel")
            )
    ...
    if current_df_excel is not None:
        ...
        time_shift_days = (
            folder_start_datetime - current_min_timestamp_excel
        ).total_seconds() / (3600 * 24)

        df_excel_with_shifted_time = current_df_excel.with_columns(
            (pl.col("continuous_time_d_excel") + time_shift_days)
            .alias("absolute_continuous_time_d_excel")
        )
        ...
        if df_excel_filtered_aligned.is_empty():
            st.warning(f"No data available for {selected_folder_name}")
        else:
            ...
            ax1_excel.plot(
                df_excel_filtered_aligned["absolute_continuous_time_d_excel"].to_numpy(),
                df_excel_filtered_aligned[current_knots_col_excel].to_numpy(),
                label=current_knots_col_excel,
                color="blue",
                alpha=0.8
            )

            ax1_excel.set_xlabel("Time [d]")
            ax1_excel.set_ylabel(current_knots_col_excel, color="blue")
            ...
            ax2_excel.scatter(
                df_excel_filtered_aligned["absolute_continuous_time_d_excel"].to_numpy(),
                df_excel_filtered_aligned[current_udir_col_excel].to_numpy(),
                color='red',
                s=10
            )

            ax2_excel.set_ylabel("Wind Direction", color="red")
            ax2_excel.set_ylim(0, 360)

            # ✅ COMPASS TICKS (UNCHANGED FROM YOUR ORIGINAL INTENT)
            ...
            if current_hs_col_excel:
                ...
                if val_hs_at_time is not None:
                    ...
                    st.write("**Current Wave Height:** No wave height available")
            ...
            if current_tp_col_excel:
                ...
                if val_tp_at_time is not None:
                    ...
                    st.write("**Current Wave Period:** No wave period available")

            # =========================
            # SMOOTHED PLOT (UNCHANGED STRUCTURE)
            # =========================
            if show_conditionally_smoothed_wind:
                st.header("Wind Forecast")
                # =========================
                ...
                df_plot_smoothed = df_excel_filtered_aligned.with_columns([
                    ...
                    ((pl.col("absolute_continuous_time_d_excel") > selected_time_d) &
                    (pl.col("absolute_continuous_time_d_excel") <= selected_time_d + 1))
                    .alias("is_24h_forecast"),

                    (pl.col("absolute_continuous_time_d_excel") > selected_time_d + 1)
                    .alias("is_late_forecast"),
                ])

                # =========================
                # STEP 2: APPLY CORRECT SMOOTHING
                # =========================
                df_plot_smoothed = df_plot_smoothed.with_columns([

                    # WIND SPEED
                    pl.when(pl.col("is_past"))
                    .then(pl.col(current_knots_col_excel))

                    .when(pl.col("is_24h_forecast"))
                    .then(
                        pl.col(current_knots_col_excel)
                        .rolling_mean(window_size=10, center=True, min_periods=1)
                    )

                    .otherwise(
                        pl.col(current_knots_col_excel)
                        .rolling_mean(window_size=20, center=True, min_periods=1)
                    )
                ...
                # =========================
                # STEP 3: PLOT (UNCHANGED STYLE)
                # =========================
                fig_smoothed_wind, ax1_smoothed_wind = plt.subplots(figsize=(18, 6))

                ax1_smoothed_wind.plot(
                    df_plot_smoothed["absolute_continuous_time_d_excel"].to_numpy(),
                    df_plot_smoothed[f"{current_knots_col_excel}_smoothed"].to_numpy(),
                    color="blue",
                    alpha=0.8
                )

                ax1_smoothed_wind.set_xlabel("Time [d]")
                ax1_smoothed_wind.set_ylabel(current_knots_col_excel, color="blue")
                ...
                ax2_smoothed_wind.scatter(
                    df_plot_smoothed["absolute_continuous_time_d_excel"].to_numpy(),
                    df_plot_smoothed[f"{current_udir_col_excel}_smoothed"].to_numpy(),
                    color="red",
                    s=10
                )

                ax2_smoothed_wind.set_ylabel("Wind Direction", color="red")
                ax2_smoothed_wind.set_ylim(0, 360)

                # COMPASS TICKS (UNCHANGED)
...
if link_mb_stats: # Check if stats were computed (means varyingWL and mb_cols_present)
    ...
    with col1:
      st.image(berth_wind_img, width=img_width)
    with col2:
      st.image(mooring_layout_img, width=img_width)
...
if excursion_stats:
    ...
    # Set desired width for both images (pixels)
    img_width = 300
    st.image(ship_coords_img, caption='Defined Coordinate System', width=img_width)
